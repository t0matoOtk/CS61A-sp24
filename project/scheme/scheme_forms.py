...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        symbol, parameter = signature.first, signature.rest
        body = expressions.rest
        formals = Pair(parameter, body)
        lambdaFrame = do_lambda_form(formals, env)
        env.define(symbol, lambdaFrame)
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    ret = True
    while (expressions and is_scheme_true(ret)):
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    ret = False
    while (expressions and is_scheme_false(ret)):
        ret = scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return ret
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        print(f"DEBUG: {expressions}, {clause}, {test}")
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            clause = clause.rest
            if (clause):
                return eval_all(clause, env)
            else:
                return test
            # END PROBLEM 13
        expressions = expressions.rest
